...
function calculateShenPalace(month, day, timePeriod, gender) {
    ...
    // 男命从寅宫起正月,女命从申宫起正月
    const mingPalace = calculateMingPalace(month, day, timePeriod);
    const timeIndex = window.TIME_PERIODS.findIndex(t => t.name === timePeriod);
...
function calculateMainStars(timePeriod, keCut, mingPalace) {
    const timeIndex = window.TIME_PERIODS.findIndex(t => t.name === timePeriod);
    const keIndex = window.TIME_KE_CUTS.findIndex(k => k.name === keCut);
...
if (typeof window !== 'undefined') {
    window.calculateFineChart = calculateFineChart;
    window.generateCharacter = generateCharacter;
    window.generateCharacterBio = generateCharacterBio;
    // 数据已在era-data.js中导出到window
}
